...
Both generators build their XML on top of `jmx_xml.py`, which owns the low-level element helpers (`sub`, `string_prop`, `bool_prop`, `jmx_to_string`) and `build_test_plan_scaffold(name)` — the shared `jmeterTestPlan` → `TestPlan` → disabled `TestFragmentController` structure every generated file starts with. `jmx_generator.py` and `build_scenario_jmx.py` each append their own children (sampler/header manager vs. `IncludeController`s) to the `fragment_tree` the scaffold returns.
