...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="KEY=VALUE",
        help="Naming template variable, e.g. --var project=Widget. Repeatable.",
    )
    parser.add_argument(
        "--out-dir",
        default=".",
...
```bash
python3 har_to_jmx.py login.har --var project=Widget --var product=Gadget
```

```bash
python3 har_to_jmx.py login.har hosts.csv --var project=Widget --var product=Gadget
```

```bash
python3 har_to_jmx.py login.har hosts.csv --var project=Widget --var product=Gadget --out-dir generated --verbose
```

```bash
python3 har_to_jmx.py login.har --var project=Widget --var product=Gadget --config gadget-config.json --out-dir generated
```
...
| `--var key=value` | Depends on `naming.template` | Supplies a naming template variable, e.g. `--var project=Widget`. Repeatable. The default `naming.template` needs `project` and `product`; a custom template may need different variables, or none. |
...
```text
Widget_Gadget_GET_recent.jmx
Widget_Gadget_POST_token.jmx
```
...
```text
Widget_Gadget_GET_users.jmx
Widget_Gadget_GET_users_2.jmx
Widget_Gadget_GET_users_3.jmx
```
...
Replace the section (from `## Generate a Scenario JMX` through the `Widget_Gadget_GET_recent.jmx` example, not including the following `The scenario builder applies...` paragraph) with:
...
```bash
python3 build_scenario_jmx.py recent.har Recent_Transmittions --var project=Widget --var product=Gadget
```

```bash
python3 build_scenario_jmx.py recent.har Recent_Transmittions --var project=Widget --var product=Gadget --out-dir generated --verbose
```

```bash
python3 build_scenario_jmx.py recent.har Recent_Transmittions --var project=Widget --var product=Gadget --config gadget-config.json --out-dir generated
```
...
```text
Widget_Gadget_Recent_Transmittions.jmx
```
...
```bash
python3 har_to_jmx.py recording.har Widget Gadget hosts.csv
```

4. Generate a scenario file:

```bash
python3 build_scenario_jmx.py recording.har Widget Gadget ScenarioName
```
```
...
```bash
python3 har_to_jmx.py recording.har hosts.csv --var project=Widget --var product=Gadget
```

4. Generate a scenario file:

```bash
python3 build_scenario_jmx.py recording.har ScenarioName --var project=Widget --var product=Gadget
```
```
...
python3 har_to_jmx.py /tmp/smoke.har --var project=Widget --var product=Gadget --out-dir /tmp/smoke-out
python3 build_scenario_jmx.py /tmp/smoke.har Recent_Transmittions --var project=Widget --var product=Gadget --out-dir /tmp/smoke-out
ls /tmp/smoke-out
rm -rf /tmp/smoke.har /tmp/smoke-out
```

Expected: both commands succeed (exit 0); `/tmp/smoke-out` contains `Widget_Gadget_GET_users.jmx` and `Widget_Gadget_Recent_Transmittions.jmx`, matching the README's documented filenames.
